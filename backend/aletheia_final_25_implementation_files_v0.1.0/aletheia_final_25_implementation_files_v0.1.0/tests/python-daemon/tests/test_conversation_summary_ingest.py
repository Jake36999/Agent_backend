...
class ConversationSummaryIngestTests(unittest.TestCase):
    def test_explicit_user_decision_becomes_decision_memory(self):
        ...
        result = ingestor.build_memory_candidate(
    ...
    def test_untested_hypothesis_is_rejected(self):
        ingestor = ConversationSummaryIngestor()
        result = ingestor.build_memory_candidate(conversation_events=[{"type": "hypothesis", "content": "Maybe X."}])
        self.assertFalse(result["write_allowed"])

    def test_secret_like_content_is_blocked(self):
        ingestor = ConversationSummaryIngestor()
        result = ingestor.build_memory_candidate(conversation_events=[{"type": "user_decision", "content": "API_TOKEN=abcdef1234567890"}])
        self.assertFalse(result["write_allowed"])

    def test_commit_only_when_allowed(self):
        ingestor = ConversationSummaryIngestor()
        memory = FakeMemory()
        candidate = ingestor.build_memory_candidate(conversation_events=[{"type": "user_decision", "content": "Keep selector internal."}])
