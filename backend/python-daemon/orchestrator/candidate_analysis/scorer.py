...
EXCLUDED_PATTERNS = ("local_tool_assist_outputs", "manifest_doctor.", "final_handoff_bundle", "session.yaml")
TEST_PATTERNS = ("tests/", "test_", "_test.", ".test.")


def tokenize(text: str | None) -> set[str]:
    return {token for token in re.findall(r"[a-z0-9_]+", (text or "").lower()) if token not in STOPWORDS and len(token) > 1}
...
def parse_manifest_candidates(manifest_csv: str | None) -> list[dict[str, Any]]:
    ...
    try:
        ...
        if reader.fieldnames:
            for row in reader:
                path = row.get("path") or row.get("file") or row.get("relative_path") or row.get("absolute_path")
                if path:
                    candidates.append({"path": str(path).replace("\\", "/"), "row": dict(row)})
            if candidates:
                return candidates
    except csv.Error:
        pass
...
def score_path(path: str, context_tokens: set[str], rag_tokens: set[str]) -> dict[str, Any]:
    ...
    path_tokens = tokenize(normalized.replace("/", " ").replace(".", " ").replace("_", " ").replace("-", " "))
    ...
    if any(segment in lower.split("/") for segment in PENALIZED_SEGMENTS) or any(pattern in lower for pattern in PENALIZED_PATTERNS):
        score -= 8.0
        risk_notes.append("generated/cache/vendor path penalty")
    if any(pattern in lower for pattern in EXCLUDED_PATTERNS):
        score -= 20.0
        risk_notes.append("workflow artifact path penalty")
