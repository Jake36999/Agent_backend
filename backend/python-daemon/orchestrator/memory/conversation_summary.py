...
_REJECTED_TYPES = {"guess", "hypothesis", "untested_hypothesis", "raw_chat_log", "env_dump", "secret"}
_VOLATILE_KEYS = {"pid", "process_id", "tmp", "temp", "timestamp", "nonce", "session_id", "env"}
_MAX_CONTENT = 8000
_SECRET_PATTERNS = [
...
class ConversationSummaryIngestor:
    """Deterministic policy for committing verified conversation summaries."""
    ...
    def build_memory_candidate(
        ...
        *,
        ...
        for event in conversation_events or []:
            event_type = str(event.get("type") or event.get("basis") or "").lower()
            if event_type in _REJECTED_TYPES:
                return self._rejected("Unverified or unsafe conversation content is not durable memory.", volatile_removed)
    ...
    def commit_if_allowed(
        ...
        *,
        ...
        return memory_service.commit_memory(
    ...
    def _infer_basis(self, events: list[dict[str, object]], source_artifacts: list[dict[str, object]] | None) -> str:
        for event in events:
            ...
            if event_type in _ALLOWED_BASES:
                return "tool_result" if event_type == "verified_file_state" else event_type
